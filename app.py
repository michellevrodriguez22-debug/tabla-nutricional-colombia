...
# Salida: PNG con fondo blanco (sin títulos adicionales)
# ============================================================
...
# Porción definida por el usuario (medida casera + equivalencia)
...
# Micronutrientes por 100 g / 100 mL (opcionales)
...
# ============================================================
# ESTILO GRÁFICO (GROSOR/COLORES/FUENTES)
# ============================================================
BORDER_W       = 6   # marco exterior
GRID_W         = 3   # líneas standard
GRID_W_THICK   = 9   # líneas gruesas (triplicado)
TEXT_COLOR     = (0,0,0)
...
FONT_MICRO     = get_font(24, bold=False)  # micronutrientes más pequeño, como norma sugiere
...
def common_rows():
    """
    Devuelve filas de macros en el orden normativo.
    - Nombres sin unidades
...
def micro_rows():
    """
    Micronutrientes:
    - Nombre sin unidades (p. ej., 'Calcio', 'Vitamina D')
    - Unidades van a la derecha del valor (mg o µg)
    """
    rows = []
    for (name, unit), v100 in vm_values.items():
        vpp = vm_pp[(name, unit)]
...
def column_labels():
    # Solo títulos; no repetir cantidades
...
def draw_fig1():
    ...
    W = 1400
    header_h = 150  # título centrado + bloque porción a la izquierda
    gap_after_title = 8
    colhdr_h = 70
    calories_h = ROW_H
    foot_h = 110
    ...
    H = (
        ...
        + header_h
        + gap_after_title
        + GRID_W_THICK      # línea gruesa bajo cabecera
        + colhdr_h
        + GRID_W            # línea fina bajo encabezados
        + calories_h
        + GRID_W_THICK      # línea gruesa bajo calorías
        + body_rows_h
        + GRID_W_THICK      # línea gruesa antes del pie
        + foot_h
    )
    ...
    # ---------- Bloque porción (izquierda, no centrado) ----------
    y0 = BORDER_W + 10 + th + 6
    d.text((BORDER_W + CELL_PAD_X, y0 + 12),
           f"Tamaño por porción: {household_name} ({int(round(portion_size))} {portion_unit})",
           fill=TEXT_COLOR, font=FONT_SMALL)
    d.text((BORDER_W + CELL_PAD_X, y0 + 12 + 36),
           f"Número de porciones por envase: Aprox. {fmt_g(servings_per_pack,0)}",
           fill=TEXT_COLOR, font=FONT_SMALL)
    ...
    # Verticales internas desde aquí (no cruzan la cabecera)
    data_top = y
    # data_bottom se ajustará antes del pie
    # (lo calculamos al final con precisión)
    # De momento, trazaremos luego para evitar que "se pasen"
    # Guardamos las x:
    x_v2 = col_x[2]
    x_v3 = col_x[3]

    # ---------- Fila Calorías (misma fila, solo valores; sin repetir títulos) ----------
    y += 1
    # línea gruesa arriba de calorías
    draw_hline(d, BORDER_W, W-BORDER_W, y, TEXT_COLOR, GRID_W_THICK)
    y += 2

    d.text((BORDER_W + CELL_PAD_X, y + (ROW_H//2) - 14),
    ...
    # Separador nutrientes/micro (grueso)
    draw_hline(d, BORDER_W, W-BORDER_W, y, TEXT_COLOR, GRID_W_THICK)

    # ---------- Micronutrientes (pequeños; nombre sin unidad, valor con unidad) ----------
    if show_micro:
    ...
    # Línea gruesa antes del pie (acá definimos bottom para verticales)
    draw_hline(d, BORDER_W, W-BORDER_W, y, TEXT_COLOR, GRID_W_THICK)

    # Ahora sí dibujamos las verticales con longitud exacta:
    data_bottom = y  # hasta justo antes del pie
    draw_vline(d, x_v2, data_top, data_bottom, TEXT_COLOR, GRID_W)
    draw_vline(d, x_v3, data_top, data_bottom, TEXT_COLOR, GRID_W)
...
def draw_fig3():
    ...
    # Título centrado + bloque porción
    title = "Información Nutricional"
    tw, th = text_size(d, title, FONT_TITLE)
    d.text(((W-tw)//2, BORDER_W + 10), title, fill=TEXT_COLOR, font=FONT_TITLE)
    d.text((BORDER_W + CELL_PAD_X, BORDER_W + 10 + th + 12),
           f"Tamaño por porción: {household_name} ({int(round(portion_size))} {portion_unit})",
           fill=TEXT_COLOR, font=FONT_SMALL)
    d.text((BORDER_W + CELL_PAD_X, BORDER_W + 10 + th + 12 + 36),
           f"Número de porciones por envase: Aprox. {fmt_g(servings_per_pack,0)}",
           fill=TEXT_COLOR, font=FONT_SMALL)
    ...
    # Verticales desde data_top hasta y (antes del pie)
    draw_vline(d, col_x[2], data_top, y, TEXT_COLOR, GRID_W)
    draw_vline(d, col_x[3], data_top, y, TEXT_COLOR, GRID_W)
...
# ============================================================
# FIGURA 4 — TABULAR (cuadrícula completa)
# ============================================================
def draw_fig4():
    rows_nutri = common_rows()
    rows_micro = micro_rows()
    ...
    # Columnas más “tabla”: 1ra ancha, 2da y 3ra equilibradas
    ...
    # Verticales completas (cuadrícula hasta antes del pie)
    data_top = y
    # Calorías
    y += 1
    draw_hline(d, BORDER_W, W-BORDER_W, y, TEXT_COLOR, GRID_W_THICK)
    ...
    # Ahora dibujamos las verticales completas (cuadrícula)
    data_bottom = y
...
# ============================================================
def draw_fig5():
    # Texto por porción con paréntesis “por 100”
    items = []
    ...
    W = 1600
    H = 640 if len(items) <= 11 else 780
    img = Image.new("RGB", (W,H), BG_WHITE)
    d = ImageDraw.Draw(img)
    d.rectangle([0,0,W-1,H-1], outline=TEXT_COLOR, width=BORDER_W)

    # Primera línea informativa (no título grande)
    left_x = BORDER_W + 28
    y = BORDER_W + 28
    d.text((left_x, y),
           f"Información nutricional (por porción): Tamaño por porción: {household_name} ({int(round(portion_size))} {portion_unit})   •   Número de porciones por envase: Aprox. {fmt_g(servings_per_pack,0)}",
    ...
    y += 10
    d.text((left_x, y), f"No es fuente significativa de {footnote_tail.strip().rstrip('.')}",
           fill=TEXT_COLOR, font=FONT_SMALL)
