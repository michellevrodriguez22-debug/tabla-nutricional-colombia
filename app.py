...
# ---- Reglas de redondeo (Res. 810/2021 Tabla 1) y "cantidades no significativas" (Tabla 2) ----
def round_kcal(v):
    # 810: valores < 5 kcal pueden declararse como 0
    if v < 5:
        return 0
    # energía se declara como entero
    return int(round(v))

def round_g(v):
    # Cantidades no significativas (Tabla 2)
    if v < 0.005:  # ultra-pequeños por seguridad numérica
        v = 0.0
    # regla general por magnitud
    av = abs(v)
    if av < 0.5:
        # si aplica “no significativa” según nutriente se maneja en cada caso
...
def round_mg(v_mg):
    # Cantidad no significativa (Tabla 2): Sodio < 5 mg -> 0 mg
    if v_mg < 5:
        return 0
    return int(round(v_mg))

def fmt_g(x):
    # imprimir sin ceros de cola
...
# Validación no impresa: contiene edulcorantes
...
# Energía (antes de redondear/no significativas)
...
# Aplicar “no significativas” por nutriente (Tabla 2)
...
trans_fat_100_mg_r= round_mg(nonsig_zero_mg("Sodio", 0))  # placeholder not used for trans
...
# trans por 100 está en mg de entrada; convertir a g para “no significativa” y volver a mg
...
# Por porción (aplicar “no significativas” y redondeo)
...
# Micronutrientes por porción (redondeo simple: mg entero; µg con 1 decimal no es usual → entero)
vm_pp = {}
vm_values_rounded = {}
...
# ============================================================
# % kcal de la porción que provienen del nutriente
def pct_kcal_from(nutrient_kcal, total_kcal_pp):
    if total_kcal_pp <= 0:
        return 0.0
    return 100.0 * nutrient_kcal / total_kcal_pp

sat_pct = pct_kcal_from(9 * (sat_fat_pp if sat_fat_pp > 0 else 0), max(kcal_pp_raw, 1e-9))
trans_pct = pct_kcal_from(9 * ((trans_fat_pp_mg or 0)/1000.0), max(kcal_pp_raw, 1e-9))
sug_add_pct = pct_kcal_from(4 * (sug_added_pp if sug_added_pp > 0 else 0), max(kcal_pp_raw, 1e-9))
...
# ============================================================
# FILAS (usando redondeo aplicado arriba)
# ============================================================
def common_rows():
    rows = [
...
def draw_fig1():
    ...
    d.text((BORDER_W + CELL_PAD_X, y0 + 16 + 36), f"Número de porciones por envase: {int(round(servings_per_pack))}", fill=TEXT_COLOR, font=FONT_SMALL)
...
def draw_fig3():
    ...
    d.text((BORDER_W + CELL_PAD_X, BORDER_W + 10 + th + 16 + 36), f"Número de porciones por envase: {int(round(servings_per_pack))}", fill=TEXT_COLOR, font=FONT_SMALL)
