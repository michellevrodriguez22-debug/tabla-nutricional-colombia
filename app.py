...
for i, vm in enumerate(selected_vm):
    ...
    col = vm_col1 if i % 2 == 0 else vm_col2
    with col:
...
def common_rows():
    return [
        ("Grasa total", f"{fmt_g(fat_total_100)} g", f"{fmt_g(fat_total_pp)} g", 0, False, False),
        ("  Grasa saturada", f"{fmt_g(sat_fat_100)} g", f"{fmt_g(sat_fat_pp)} g", 1, True, False),
        ("  Grasas trans", f"{fmt_mg(trans_fat_100_mg)} mg", f"{fmt_mg(trans_fat_pp_mg)} mg", 1, True, False),
        ("Carbohidratos totales", f"{fmt_g(carb_100)} g", f"{fmt_g(carb_pp)} g", 0, False, False),
        ("  Fibra dietaria", f"{fmt_g(fiber_100)} g", f"{fmt_g(fiber_pp)} g", 1, False, False),
        ("  Azúcares totales", f"{fmt_g(sug_total_100)} g", f"{fmt_g(sug_total_pp)} g", 1, False, False),
        ("  Azúcares añadidos", f"{fmt_g(sug_added_100)} g", f"{fmt_g(sug_added_pp)} g", 1, True, False),
        ("Proteína", f"{fmt_g(protein_100)} g", f"{fmt_g(protein_pp)} g", 0, False, False),
...
# ============================================================
# FIGURA 1 — VERTICAL ESTÁNDAR (corregida)
# ============================================================
def draw_fig1():
    ...
    W, header_h, gap_after_title, colhdr_h, calories_h, foot_h = 1400, 140, 10, 70, ROW_H, 110
    ...
    d.text((BORDER_W + CELL_PAD_X, y0 + 52), f"Número de porciones por envase: {int(round(servings_per_pack))}", fill=TEXT_COLOR, font=FONT_SMALL)
    ...
    # ==== Calorías (kcal) con celdas combinadas ====
    y += 1
    ...
    kc100, kcpp = fmt_kcal(kcal_100), fmt_kcal(kcal_pp)
    ...
    draw_hline(d, BORDER_W, W-BORDER_W, y, TEXT_COLOR, GRID_W_THICK)
# ============================================================
# FIGURA 3 — SIMPLIFICADO (Calorías combinadas)
# ============================================================
def draw_fig3():
    rows = [
        ("Grasa total", f"{fmt_g(fat_total_100)} g", f"{fmt_g(fat_total_pp)} g", 0, False),
        ("  Grasa saturada", f"{fmt_g(sat_fat_100)} g", f"{fmt_g(sat_fat_pp)} g", 1, True),
        ("  Grasas trans", f"{fmt_mg(trans_fat_100_mg)} mg", f"{fmt_mg(trans_fat_pp_mg)} mg", 1, True),
        ("Carbohidratos totales", f"{fmt_g(carb_100)} g", f"{fmt_g(carb_pp)} g", 0, False),
        ("  Azúcares añadidos", f"{fmt_g(sug_added_100)} g", f"{fmt_g(sug_added_pp)} g", 1, True),
        ("Proteína", f"{fmt_g(protein_100)} g", f"{fmt_g(protein_pp)} g", 0, False),
        ("Sodio", f"{fmt_mg(sodium_100_mg)} mg", f"{fmt_mg(sodium_pp_mg)} mg", 0, True)
    ]

    W, header_h, colhdr_h, gap_after_title, foot_h = 1200, 140, 70, 10, 110
    H = BORDER_W*2 + header_h + gap_after_title + GRID_W_THICK + colhdr_h + GRID_W + ROW_H + GRID_W_THICK + len(rows)*ROW_H + GRID_W_THICK + foot_h
    col_x = [BORDER_W, BORDER_W + int(W*0.52), BORDER_W + int(W*0.78), W - BORDER_W]
    ...
    d.text((BORDER_W + CELL_PAD_X, BORDER_W + 10 + th + 52), f"Número de porciones por envase: {int(round(servings_per_pack))}", fill=TEXT_COLOR, font=FONT_SMALL)
    ...
    data_bottom = H - BORDER_W - foot_h - GRID_W_THICK
    for cx in col_x[1:]:
        draw_vline(d, cx, data_top, data_bottom, TEXT_COLOR, GRID_W)
    ...
    for label, v100, vpp, indent, bold in rows:
        y += 1
        draw_hline(d, BORDER_W, W-BORDER_W, y, TEXT_COLOR, GRID_W)
        f_lbl, f_val = (FONT_LABEL_B if bold else FONT_LABEL), (FONT_LABEL_B if bold else FONT_LABEL)
        x_label = BORDER_W + CELL_PAD_X + indent*28
        y_text = y + (ROW_H//2) - 14
        d.text((x_label, y_text), label, fill=TEXT_COLOR, font=f_lbl)
        wv100,_ = text_size(d, v100, f_val)
        wvpp,_  = text_size(d, vpp,  f_val)
        d.text((col_x[2]-CELL_PAD_X-wv100, y_text), v100, fill=TEXT_COLOR, font=f_val)
        d.text((col_x[3]-CELL_PAD_X-wvpp,  y_text), vpp,  fill=TEXT_COLOR, font=f_val)
        y += ROW_H
...
# ============================================================
# FIGURA 4 — TABULAR (Calorías combinadas)
# ============================================================
def draw_fig4():
    rows_nutri, rows_micro = common_rows(), micro_rows()
    show_micro = len(rows_micro) > 0

    W, header_h, colhdr_h, gap_after_title, foot_h = 1500, 140, 70, 10, 110
    body_h = len(rows_nutri)*ROW_H + (len(rows_micro)*ROW_H_MICRO if show_micro else 0)
    H = BORDER_W*2 + header_h + gap_after_title + GRID_W_THICK + colhdr_h + GRID_W + ROW_H + GRID_W_THICK + body_h + GRID_W_THICK + foot_h
    ...
    draw_hline(d, BORDER_W, W-BORDER_W, y, TEXT_COLOR, GRID_W)
    for cx in col_x[1:]:
        draw_vline(d, cx, y, H - BORDER_W - foot_h - GRID_W_THICK, TEXT_COLOR, GRID_W)
...
def draw_fig5():
    ...
    pair("Grasa total", f"{fmt_g(fat_total_pp)} g", f"{fmt_g(fat_total_100)} g")
    pair("Grasa saturada", f"{fmt_g(sat_fat_pp)} g", f"{fmt_g(sat_fat_100)} g")
    pair("Grasas trans", f"{fmt_mg(trans_fat_pp_mg)} mg", f"{fmt_mg(trans_fat_100_mg)} mg")
    pair("Carbohidratos totales", f"{fmt_g(carb_pp)} g", f"{fmt_g(carb_100)} g")
    pair("Azúcares añadidos", f"{fmt_g(sug_added_pp)} g", f"{fmt_g(sug_added_100)} g")
    pair("Proteína", f"{fmt_g(protein_pp)} g", f"{fmt_g(protein_100)} g")
    ...
    d = ImageDraw.Draw(img)
    d.rectangle([0,0,W-1,H-1], outline=TEXT_COLOR, width=BORDER_W)
    left_x, y = BORDER_W + 28, BORDER_W + 28
    d.text((left_x, y), f"Información nutricional (por porción): Tamaño por porción: {household_name} ({int(round(portion_size))} {portion_unit})   •   Número de porciones por envase: {int(round(servings_per_pack))}", fill=TEXT_COLOR, font=FONT_SMALL_B)
    y += 52
    maxw = W - left_x - 30
    s = "  •  ".join(items)
    words = s.split(" ")
    ...
    for w in words:
        t = (line + " " + w).strip()
        if text_size(d, t, FONT_LABEL)[0] <= maxw: line = t
...
with left:
    if format_choice.startswith("Fig. 1"): img_prev = draw_fig1()
    elif format_choice.startswith("Fig. 3"): img_prev = draw_fig3()
    elif format_choice.startswith("Fig. 4"): img_prev = draw_fig4()
    else: img_prev = draw_fig5()
