...
def round_g(v):
    ...
    Regla práctica por magnitud:
      ...
      - [0.5, 10) → 1 decimal
      - [10, 100) → 1 decimal (para consistencia visual; opcional entero si lo prefieres)
      - >=100 → entero
    """
    av = abs(v)
    if av >= 100:
...
# Micronutrientes por porción (mg/µg a entero)
vm_pp = {}
vm_values_rounded = {}
...
def draw_calories_combined_row(d, W, y, col_x, kcal_100_txt, kcal_pp_txt):
    """
    Dibuja el bloque "Calorías (kcal)" como UNA sola fila alta (celda combinada para el título),
    sin línea horizontal media atravesando el texto.

    Estructura:
      ┌───────────────────────────────────────────────────────────────────────┐
      │ Calorías (kcal)          |   Por 100 g/mL (valor)   |   Por porción  │
      └───────────────────────────────────────────────────────────────────────┘

    - Se dibuja UNA línea gruesa arriba y UNA gruesa abajo.
    - No se dibuja línea horizontal a mitad que corte el texto.
    - Los valores a derecha se centran verticalmente en la misma fila.
    """
    # línea gruesa arriba
